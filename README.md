...
```markdown
【任務】你是一位專業的 AI 教學內容編輯。請將以下 YouTube 影片逐字稿，整理成一份「操作說明書等級」的詳細操作講義。

【格式規範】
- 標題：與影片標題完全一致（不加任何前綴）
- 結構：使用 ## 二級標題分章節，## 小節使用 ### 三級標題
- 內容要求：
  - 每個操作步驟必須寫出完整路徑（如：點選左側選單 > 設定 > 進階）
  - 擷取講師的經典金句並加粗
  - 標記地雷與注意事項（使用 > [!IMPORTANT] 格式）
  - 保留原始的工具名稱、快捷鍵、網址等具體細節
- 結尾：不加任何版權宣告、Padlet 連結或個人署名
- 無 YAML Frontmatter

【版權說明】
此講義內容整理自影片，版權歸原影片創作者所有，僅供學習參考。
...
**生圖提示詞：**
請根據《Puti 老師人物動作素材包.jpg》、《Puti-AI 機器人 GiGi 表情包.jpg》的角色造型，以及《極簡手繪簡報外框範本.png》的風格。極簡手繪塗鴉風格，手繪簡報外框，純白背景，黑色線稿。Puti老師與GiGi機器人同台出現，[詳細描述Puti老師與GiGi機器人在畫面中如何彼此互動，如何與軟體畫面、按鈕、符號或巨型工具進行動態連結的中文細節]。使用柔和藍色和橘色局部點綴。乾淨排版，預留標題與文案文字空間。。
```
